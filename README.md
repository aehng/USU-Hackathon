...
**⚠️ README Keeper Rule:** Eli is the README keeper. All changes to `DEMO_USER_ID`, `API_BASE_URL`, and port mappings must go through Eli to prevent merge conflicts. **Ask Eli before changing frontend constants or backend ports.**
...
| Noah | Backend — Local LLM Integration + Cloudflare Tunnels + API Routes |
| Clayton | Backend — Database Schema + Analysis Engine |
| Max | Frontend & Validation — Dashboard + Charts + C-based JSON Filter |
...
| API Docs | http://192.168.1.9:8000/docs (auto-generated by FastAPI) |
...
| VM IP | 192.168.1.9 |
| Frontend | http://192.168.1.9:5173 |
| Backend API | http://192.168.1.9:8002 |
| API Docs | http://192.168.1.9:8002/docs (auto-generated by FastAPI) |
| LLM Backend | https://flairup.dpdns.org (Noah's LLM) |
...
| Fri 7:00pm | ⭐ 3-way Integration checkpoint — Noah + Clayton + Max finalize JSON contracts. (Max starts C-filter dev post-7pm) |
| Fri 9:00pm | Voice → backend → C-filter → DB working end to end |
